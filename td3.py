...
def td3(
        ...
        epochs, start_steps, steps_per_epoch, policy_update_delay, inc_ep,
    ...
    actor_critic = core.get_td3_actor_critic(ac_type)
    # Main outputs from computation graph
    with tf.variable_scope('main'):
        pi, q1, q2, q, q1_pi, q2_pi, v1, v2, v = actor_critic(x_ph, a_ph, z_ph, **ac_kwargs)
    
    # Target networks
    with tf.variable_scope('target'):
        _, _, _, _, _, _, _, _, v_targ = actor_critic(x2_ph, a_ph, z_ph, **ac_kwargs)
...
#    print("\n".join([var.name for var in get_vars("")]))
#    exit(0)

    # Bellman backup for Q and V function
    q_backup = tf.stop_gradient(r_ph + gamma*(1-d_ph)*v_targ)
    v1_backup = tf.stop_gradient(q1_pi)
    v2_backup = tf.stop_gradient(q2_pi)
    ...
    v1_loss = 0.5 * tf.reduce_mean((v1 - v1_backup)**2)
    v2_loss = 0.5 * tf.reduce_mean((v2 - v2_backup)**2)
    v_loss = v1_loss + v2_loss
    ...
    train_value_op = value_optimizer.minimize(value_loss, var_list=get_vars('main/q1') + get_vars('main/q2') +
                                                                   get_vars('main/v1') + get_vars('main/v2'))
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            for index in range(ep_len):
                ...
                # Policy update
                if (index + 1) % policy_update_delay == 0 or index + 1 == ep_len:
...
if __name__ == '__main__':
    ...
    import json
    model_config = json.load(open(util.MODEL_CONFIG_DIR + "td3/" + args.model_config))
    ...
    td3(
        ...
        policy_update_delay=model_config["policy_update_delay"],
